...
const Login = () => {
  const { login, loginWithGoogle, loginWithGithub, handleRedirectResult } =
    useAuth();
  ...
  const handleSocialLogin = async (provider: string) => {
    ...
    try {
      ...
      setErrors({
        general:
          code || `Failed to sign in with ${provider}. Please try again.`,
      });
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6 md:p-8 shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex items-center justify-between flex-wrap">
                ...
                <Link
                  to="/forgot-password"
                  className="text-netflix-red hover:text-netflix-red/80 text-sm font-medium transition-colors"
                >
                  Forgot password?
                </Link>
              </div>
