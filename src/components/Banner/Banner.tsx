...
const Banner = ({ contentType = 'all' }: BannerProps) => {
    ...
    const handlePrev = () => setCurrentIndex((currentIndex - 1 + totalMovies) % totalMovies);
    const handleNext = () => setCurrentIndex((currentIndex + 1) % totalMovies);
    ...
    // on mobile use poster_path, fallback to backdrop_path if poster is null
        ...
        <div className="">
            ...
            {(loading || !movie) ? (
                ...
                <section className="relative">
                        ...
                        >
                            ...
                            <div className="bg-movie-details md:min-h-[85dvh] min-h-[50dvh]">
                                <div
                                    ...
                                    style={{ backgroundImage: bgLoaded === currentIndex ? `url(${getImageUrl(bgPath)})` : 'none' }}
                                >
                    ...
                    {totalMovies > 1 && (
                        <>
                            <button onClick={handlePrev} className="cursor-pointer absolute left-5 top-1/2 -translate-y-1/2 z-20 bg-black/50 hover:bg-black/80 text-white p-2 rounded-full transition">
                                <ChevronLeft className="size-8" />
                            </button>
                            <button onClick={handleNext} className="cursor-pointer absolute right-5 top-1/2 -translate-y-1/2 z-20 bg-black/50 hover:bg-black/80 text-white p-2 rounded-full transition">
